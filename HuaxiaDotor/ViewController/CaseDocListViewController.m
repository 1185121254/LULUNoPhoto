...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    [cell.imageView sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"默认婷婷"]];
...
-(void)creatSelectedDoc:(NSMutableArray *)arry{
    ...
    for (NSInteger i = 0; i < arry.count; i++) {
        ...
        NSString *yrl = [NSString stringWithFormat:@"%@%@",NET_URLSTRING,[dic objectForKey:@"picFileName"]];
        NSString *url = [yrl stringByReplacingOccurrencesOfString:@"\\" withString:@"/"];
        [image sd_setImageWithURL:[NSURL URLWithString:url] placeholderImage:[UIImage imageNamed:@"默认婷婷"]];
    ...
    if ([self.from isEqualToString:@"CommPerson"]) {
        ...
        [[NIMSDK sharedSDK].teamManager addUsers:arryInvite toTeam:self.teamID postscript:@"请你入群" completion:^(NSError *error, NSArray *members) {
            if (!error) {
                ...
                [RequestManager postWithURLStringALL:[NSString stringWithFormat:@"%@/api/Doctor/DiscussionMemberAdd",NET_URLSTRING] heads:DICTIONARY_VERIFYCODE parameters:dicRequset viewConroller:self success:^(id responseObject, BOOL ifTimeout) {
