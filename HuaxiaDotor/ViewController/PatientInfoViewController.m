...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    for (int i = 0 ; i < self.arryPic.count; i++) {
        //传递数据给浏览器
        MJPhoto *photo = [[MJPhoto alloc] init];
        if ([self.arryPic[i] isKindOfClass:[NSString class]]) {
            NSString *picUrl = self.arryPic[i];
...
//    http://www.yanketong.com:90/api/His/GetPatientExamInfo?ApplyExamCode=10907675&ExamCode=GV186834&DataType=4
