...
-(void)viewWillDisappear:(BOOL)animated{
    ...
    if (![_state isEqualToString:@"3"]) {
        NSDictionary *dicU = @{@"fromAccount":self.patientModel.patientIM,@"toAccount":kDoctorIM};
        [RequestManager getWithURLStringALL:[NSString stringWithFormat:@"%@/api/Share/SetQuestionMessageIsRead",NET_URLSTRING] heads:DICTIONARY_VERIFYCODE parameters:dicU viewConroller:self success:^(id responseObject) {
...
-(void)getWebKeShi{
    ...
    [RequestManager getWithURLStringALL:[NSString stringWithFormat:@"%@/api/Share/GetHosDept",NET_URLSTRING] heads:DICTIONARY_VERIFYCODE parameters:dicData viewConroller:self success:^(id responseObject) {
...
- (void)UUInputFunctionView:(UUInputFunctionView *)funcView sendMessage:(NSString *)message
{
    ...
    //判断是否是第一个条消息
    [NSString isFristMsgDoctor:_isFirstMsg Patient:[NSString getPatientTel:self.patientModel] type:@"2" viewController:self Compent:^(BOOL isFrirt) {
        _isFirstMsg = isFrirt;
    }];
...
-(void)onBTnDeop:(UIButton *)btn{
    ...
    [RequestManager getWithURLStringALL:[NSString stringWithFormat:@"%@/api/Share/QuestionTransfer",NET_URLSTRING] heads:DICTIONARY_VERIFYCODE parameters:dicData viewConroller:self success:^(id responseObject) {
