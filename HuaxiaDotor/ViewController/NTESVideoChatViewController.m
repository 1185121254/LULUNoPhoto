...
- (void)startInterface{
    ...
    self.lblOver.hidden = YES;
    self.lblClose.hidden = YES;
    self.closeVoice.hidden = YES;
...
- (void)waitToCallInterface{
    ...
    self.connectingLabel.text = [@"aaaaa" stringByAppendingString:@"的来电"];
    self.muteBtn.hidden = YES;
    self.switchCameraBtn.hidden = YES;
    self.disableCameraBtn.hidden = YES;
...
- (void)videoCallingInterface{
    ...
    [[NIMSDK sharedSDK].netCallManager setSpeaker:YES];
    [self.netStatusView refreshWithNetState:status];
    self.acceptBtn.hidden = YES;
    self.refuseBtn.hidden   = YES;
    self.lblOver.hidden = NO;
    self.lblClose.hidden = NO;
    self.closeVoice.hidden = NO;
    self.hungUpBtn.hidden   = NO;
...
- (void)onCall:(UInt64)callID status:(NIMNetCallStatus)status{
    ...
    switch (status) {
        case NIMNetCallStatusConnect:
        {
           ...
           weakSelf.timer =[[MZTimerLabel alloc] initWithLabel:self.durationLabel andTimerType:MZTimerLabelTypeTimer];
            weakSelf.timer.timeFormat = @"距离本次咨询结束还剩\nmm:ss";
            self.durationLabel.font = [UIFont systemFontOfSize:15];
            self.durationLabel.textColor = [UIColor redColor];
            [weakSelf.timer setCountDownTime:60*15];
            [weakSelf.timer startWithEndingBlock:^(NSTimeInterval countTime) {
                [weakSelf.timer pause];
                weakSelf.durationLabel.hidden = YES;
...
- (void)resetRemoteImage{
    ...
    self.remoteView.image = [UIImage imageNamed:@"netcall_bkg"];
}
