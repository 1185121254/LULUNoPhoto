...
- (void)UUInputFunctionView:(UUInputFunctionView *)funcView sendMessage:(NSString *)message
{
    ...
    if ([self.from isEqualToString:@"report"]) {
        //判断是否是第一个条消息
        [NSString isFristMsgDoctor:_isFirstMsg Patient:[self getPatientTel:[self.patientDoc objectForKey:@"patientIM"]] type:@"4" viewController:self Compent:^(BOOL isFrirt) {
            _isFirstMsg = isFrirt;
        }];
    }
